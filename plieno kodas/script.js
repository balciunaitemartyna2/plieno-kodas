...
function translateText(original, lang) {
  ...
  const translated = translations[lang]?.[trimmed];
...
function applyLanguage(lang) {
  ...
  document.querySelectorAll("[placeholder]").forEach((element) => {
    ...
    element.setAttribute("placeholder", lang === "lt" ? original : placeholderTranslations[lang]?.[original] || original);
  });
  ...
  if (languageCurrent) {
    languageCurrent.innerHTML = `${lang.toUpperCase()} <span aria-hidden="true">▾</span>`;
  }

  updateHeroSlide(lang);
}

function updateHeroSlide(lang = localStorage.getItem("plieno-kodas-lang") || "lt") {
...
if (form) {
  form.addEventListener("submit", (event) => {
    ...
    const name = data.get("name")?.toString().trim() || "Nenurodyta";
    const contact = data.get("contact")?.toString().trim() || "Nenurodyta";
    const service = data.get("service")?.toString().trim();
    const place = data.get("place")?.toString().trim();
    const message = data.get("message")?.toString().trim() || "Nenurodyta";
    const extraRows = [
      service ? `Paslauga: ${service}` : "",
      place ? `Objekto vieta: ${place}` : "",
    ].filter(Boolean).join("\n");

    const subject = encodeURIComponent("Užklausa dėl metalo konstrukcijos");
    const body = encodeURIComponent(
      `Sveiki,\n\nNorėčiau pasiteirauti dėl metalo konstrukcijos.\n\nVardas: ${name}\nKontaktas: ${contact}\n${extraRows ? `${extraRows}\n` : ""}Užklausa: ${message}\n\nPagarbiai`
    );

    window.location.href = `mailto:info@plienokodas.lt?subject=${subject}&body=${body}`;
  });
}
